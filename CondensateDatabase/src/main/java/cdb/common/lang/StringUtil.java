...
public final class StringUtil {
	...
	private static String trim(String str, String stripChars, int mode) {
		...
		if (mode <= 0) {
			if (stripChars == null) {
				while ((starts < ends)
						&& (Character.isWhitespace(str.charAt(starts)))) {
					starts++;
				}
				...
				while ((starts < ends)
						&& (stripChars.indexOf(str.charAt(starts)) != -1)) {
					starts++;
				}
			}
		}
		...
		if (mode >= 0) {
			if (stripChars == null) {
				while ((starts < ends)
						&& (Character.isWhitespace(str.charAt(ends - 1)))) {
					ends--;
				}
			} else if (stripChars.length() == 0) {
				return str;
				...
				while ((starts < ends)
						&& (stripChars.indexOf(str.charAt(ends - 1)) != -1)) {
					ends--;
				}
			}
		}
