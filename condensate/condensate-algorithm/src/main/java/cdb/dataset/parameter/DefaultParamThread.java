...
public class DefaultParamThread extends AbstractParamThread {
     ...
     * @param dProc          the data files parsing processor
     */
    public DefaultParamThread(int regionHeight, int regionWeight, String freqId,
                              DatasetProc dProc) {
        super();
        ...
        while ((dEntry = task()) != null) {
            ...
            for (int row = 0; row < regionRow; row++) {
                for (int col = 0; col < regionCol; col++) {
                    ...
                    double sd = Math.sqrt(
                        (meanSquares.getVal(row, col) / count - mean * mean) * count / (count - 1));
