...
public class ImageDisplayAnalysis extends AbstractQcAnalysis {
    ...
    protected static void batchGenImage(DatasetProc dProc) {
        ...
        try {
            Date bDate = DateUtil.parse(begDate, DateUtil.WEB_FORMAT);
            Date eDate = DateUtil.parse(endDate, DateUtil.WEB_FORMAT);

            while (!bDate.after(eDate)) {
                DenseMatrix dMatrix = dProc
                    .read(BinFileConvntnUtil.fileAVHR(rootDir, begDate, freqId));
                if (dMatrix != null) {
                    ImageWUtil.plotGrayImage(dMatrix, rootDir + bDate + ".bmp",
                        ImageWUtil.BMP_FORMAT);
                }
