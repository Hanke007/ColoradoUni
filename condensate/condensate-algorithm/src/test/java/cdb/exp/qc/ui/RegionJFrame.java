...
public class RegionJFrame extends JFrame {
	...
	/**
	 * @param imgRootDir
	 *            the directory of region image files
	 * @param regnInfoRootDir
	 *            the directory of region information files
	 * @param regnAnmInfoFile
	 *            the file contains anomaly regions
	 * @param freqId
	 *            the frequency identification
	 * @param fContriNum
	 *            the number of field making the top contributions
	 */
	public RegionJFrame(String imgRootDir, String regnInfoRootDir, String regnAnmInfoFile, String freqId,
			int fContriNum, int sYear, boolean usedDB, String sql) {
		...
		initializeLayout(regnAnmlRep, keys, imgRootDir, regnInfoRootDir, regnAnmInfoFile, freqId, fContriNum);
	...
	protected Map<String, List<RegionAnomalyInfoVO>> parseAnomalyInfoVO(String regnAnmInfoFile, int sYear) {
		Map<String, List<RegionAnomalyInfoVO>> regnAnmlRep = new HashMap<String, List<RegionAnomalyInfoVO>>();
	...
	protected void initializeLayout(Map<String, List<RegionAnomalyInfoVO>> regnAnmlRep, List<String> keys,
			String imgRootDir, String regnInfoRootDir, String regnAnmInfoFile, String freqId, int fContriNum) {
		...
		// image layer
		String imgFile = StringUtil.isBlank(freqId) ? imgRootDir + dateStr + ".bmp"
				: imgRootDir + dateStr + "_" + freqId + ".bmp";
		...
		// button layer
		prevButton = new ControlButton("Prev", imgPanel, anmlPanel, dateStrLabel, -1, keys, regnAnmlRep, imgRootDir,
				freqId);
		nextButton = new ControlButton("Next", imgPanel, anmlPanel, dateStrLabel, +1, keys, regnAnmlRep, imgRootDir,
				freqId);
