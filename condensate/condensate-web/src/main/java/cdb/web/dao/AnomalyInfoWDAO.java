...
public interface AnomalyInfoWDAO {
    ...
    public final String ANOMALY_IN_CERTAIN_TEMPORAL_SPATIAL = "SELECT loc.LON, loc.LAT, tp.TIMESTAMP "
                                                              + "FROM VECTORS As vec "
                                                              + "JOIN LOCATIONS  AS loc  ON loc.ID = vec.LOCATIONID "
                                                              + "JOIN TIMESTAMPS As tp   ON tp.ID = vec.TIMESTAMPID "
                                                              + "WHERE tp.TIMESTAMP > ?  AND tp.TIMESTAMP < ? "
                                                              + "AND loc.ROW > ? AND loc.ROW < ? AND loc.COL > ? AND loc.COL < ? ";
